  ...
  <div class="p-4 custom-table">
    ...
    <div v-else-if="tableData.length > 0">
      <div class="grid grid-cols-12 justify-between gap-4">
        ...
        >
          <PurchasesCard :purchase="item" :listLoading />
        </div>
      </div>
      ...
      <CustomPaginator
        ...
        :label="$t('purchases.purchasesCount')"
...
import { usePurchasesDataQuery } from '../../requests/queries'
import PurchasesCard from './PurchasesCard/PurchasesCard.vue'
...
const { data: purchasesListData, isFetching: listLoading } =
  usePurchasesDataQuery(
    pageIndex,
    pageSize,
    branchId,
    searchTerm,
...
const tableData = computed(() => purchasesListData.value?.items || [])
const totalRecords = computed(() => purchasesListData.value?.totalCount || 0)
